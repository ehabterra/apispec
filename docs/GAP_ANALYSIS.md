...
- **Control-flow awareness for method/branch dispatch.** ✅ **Partially done 2026-07-12** — the specific `r.Method`-dispatch case (§2 item 8) now splits into per-method operations via a *targeted* AST detector (`detectMethodDispatch`) + line-range response scoping, deliberately **not** a general CFG. The narrow detector has since grown to cover every handler shape — closures ([#382](https://github.com/ehabterra/apispec/issues/382)) and methods ([#427](https://github.com/ehabterra/apispec/issues/427)) — and to attribute a body written a frame deeper to the arm that called it. What remains as the broader idea: a reusable control-flow/branch model for non-method conditionals (e.g. status set in one `if` arm, body in another) and same-status-different-body branches. Part of that arrived separately as the dominance/exclusivity model in `control_flow.go`; a general layer is still a good candidate to design from scratch rather than growing the narrow detector.
