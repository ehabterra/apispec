...
func TestNullableIfPointer(t *testing.T) {
	on := &APISpecConfig{Schema: SchemaConfig{NullableWhenNil: true}}
	str := func() *Schema { return &Schema{Type: "string"} }

	for _, kind := range []string{"*string", "[]string", "map[string]int", "any", "interface{}"} {
		t.Run("widens "+kind, func(t *testing.T) {
			got := nullableWhenNil(on, str(), kind, `json:"p"`)
			if len(got.AnyOf) != 2 || got.AnyOf[1].Type != "null" {
				t.Errorf("%s: got %+v, want a union admitting null — its zero value encodes as null",
					kind, got)
			}
		})
	}

	t.Run("keeps the non-null branch", func(t *testing.T) {
		got := nullableWhenNil(on, str(), "*string", `json:"p"`)
		if len(got.AnyOf) != 2 || got.AnyOf[1].Type != "null" {
	...
	for _, tc := range []struct {
		...
		{"a string", "string", `json:"p"`, on, "a string is never written as null"},
		{"a fixed-size array", "[2]string", `json:"p"`, on, "[N]T cannot be nil"},
		{"a named struct", "pkg.Thing", `json:"p"`, on, "a struct is written as an object"},
		{"a slice with omitempty", "[]string", `json:"p,omitempty"`, on, "absent when nil, never null"},
		...
		t.Run(tc.name, func(t *testing.T) {
			got := nullableWhenNil(tc.cfg, str(), tc.fieldType, tc.tag)
			if len(got.AnyOf) != 0 {
	...
	t.Run("a schema that already admits null", func(t *testing.T) {
		...
		got := nullableWhenNil(on, already, "*string", `json:"p"`)
		if len(got.AnyOf) != 2 {
	...
	t.Run("a nil schema", func(t *testing.T) {
		if got := nullableWhenNil(on, nil, "*string", `json:"p"`); got != nil {
	...
	t.Run("the description moves to the field", func(t *testing.T) {
		...
		got := nullableWhenNil(on, &Schema{Type: "string", Description: "why"}, "*string", `json:"p"`)
