...
func main() {
	...
	request, _, err := DecodeJSONSimple[TestRequest, TestRequest](nil)
	_ = request
	_ = err
	// // Set up routes with generic handlers
	// http.HandleFunc("/api/email/send", HandleRequest(handleSendEmail))
	// http.HandleFunc("/api/users", HandleRequest[CreateUserRequest, CreateUserResponse](handleCreateUser))
	// http.HandleFunc("/api/users/list", HandleRequest[struct{}, ListUsersResponse](handleListUsers))
