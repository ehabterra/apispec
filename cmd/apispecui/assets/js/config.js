...
const PATTERN_FIELDS = {
  ...
  responsePatterns: [
    ...
    ["contentTypeFromArg", "Content-type from argument", "bool", "The call states its own media type in an argument — gin's c.Data(code, contentType, data), echo's c.Blob(code, contentType, b). That media type replaces the default."],
    ["contentTypeArgIndex", "Content-type arg index", "int", "Index of the argument holding the media type when Content-type from argument is on. e.g. c.Data(200, 'application/pdf', b) → 1."],
    ...
    ["opaqueBody", "Opaque body", "bool", "This call writes BYTES rather than a Go value serialised into them — a CSV writer, io.Copy from a file, any library streaming to the response writer. The response is documented with a binary schema instead of naming a type that was never encoded."],
    ["contentTypeFromHeaderWrite", "Content type from header write", "bool", "Match this pattern against Response context → Content-type writes, and take the media type from the value the handler sets. This is how a STREAMED body is found: the handler declares what it is sending with w.Header().Set('Content-Type', …), which is one call every streaming handler makes."],
    ["rawBody", "Raw body", "bool", "This call writes its argument's bytes verbatim — w.Write(b), c.Data, c.Blob. A byte slice states no media type, so when the handler declares one (a Content-Type header write) the body is documented under THAT media type as binary, instead of as a base64 string under the JSON default. Bytes produced by a configured serializer (json.Marshal) are still that serializer's document."],
    ["typeFromResult", "Type from result", "bool", "Take the body type from the value the call RETURNS rather than from an argument — an error a handler returns, which the framework's error handler serializes: echo.NewHTTPError(code, msg) is rendered as the *echo.HTTPError it constructs."],
  ...
  paramPatterns: [
    ...
    ["requireRequestOrigin", "Require request origin", "bool", "Keep the match only when its receiver can come from the request — a value of a Request context type, reached through calls, fields, variables or parameters. url.Values is the request's query and ALSO the query of any URL the handler parses, so u.Query().Get(k) on a configuration URI is dropped. An origin that cannot be placed keeps the parameter."],
    ...SCOPE_FILTERS,
  ],
  mountPatterns: [
    ...COMMON_MATCH,
